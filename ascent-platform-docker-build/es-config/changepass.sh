...
curl -XPOST -sk -u elastic:$ES_PASSWORD $ES_URL/_xpack/security/role/zipkin -H 'Content-Type: application/json' -d "$(generate_zipkin_role_data)"

# Create developer role for read-only access to application logs
curl -XPOST -sk -u elastic:$ES_PASSWORD $ES_URL/_xpack/security/role/developer -H 'Content-Type: application/json' -d "$(generate_developer_role_data)"
...
curl -XPOST -sk -u elastic:$ES_PASSWORD $ES_URL/_xpack/security/user/$ZIPKIN_STORAGE_ELASTIC_SEARCH_USERNAME -H 'Content-Type: application/json' -d "$(generate_user_data $ZIPKIN_STORAGE_ELASTIC_SEARCH_PASSWORD)"
